...
public class Pvr_UnitySDKManagerEditor : Editor, IPreprocessBuild
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        manager.UseSinglePass = EditorGUILayout.Toggle("Use SinglePass", manager.UseSinglePass);
        if (manager.UseSinglePass != IsSinglePassEnable())
        {
            SetSinglePass(manager.UseSinglePass);
        }

        manager.ContentProtect = EditorGUILayout.Toggle("Use Content Protect", manager.ContentProtect);
        if (manager.ContentProtect)
        {
        ...
        if (GUI.changed)
        {
            ...
            {
                if (Pvr_UnitySDKManager.SDK.ContentProtect)                
